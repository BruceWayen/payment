...
public class UserCookieUtil
{
    ...
    /**
     * 保存Cookie到客户端 传递进来的user对象中封装了在登陆时填写的用户名与密码
     * 
     * @param user
     * @param response
     */
    public static void saveCookie(SysUsers user, HttpServletResponse response)
        throws Exception
    {
        // cookie的有效期至（到哪一天）
        long validTime = System.currentTimeMillis() + (cookieMaxAge * 1000);
        // MD5加密用户详细信息（其实就是把当前用户加密一下，后面判断是否是同一个用户）
        String cookieValueWithMd5 = MD5Util.md5(user.getUserName() + ":" + user.getPassWord() + ":" + validTime
            + ":" + Const.WEBKEY);
        // 将要被保存的完整的Cookie值
